//Bink.cs - author: Aaron Pfeiffer - copyright 2014 RAD Game Tools
//
//Interface to Bink from C#
//
//Layout: Contents of bink.h in comments, followed by C# version. 
//DLL Dependencies:
//
//Note: These defines are set in Assets/smcs.rsp
//#if USE_BINK2_x64
//    [DllImport("bink2w64")]
//#elif USE_BINK2
//    [DllImport("bink2w32")]
...
namespace BinkInterface
{
  public static class Bink
  {
    ...
    EXPGROUP(_NullGroup)
    #define BinkVersion "2.2e"      EXPMACRO        
     */
    public static string BinkVersion = "2.2e";
    ...
    #define BinkCustomization  16    EXPMACRO    
    #define BinkDate    "2012-06-05" EXPMACRO    
    EXPGROUP(_RootGroup)
    */
    public static string BinkDate = "2013-09-09";
    ...
    */
    ...
    public delegate int  BINKIOOPEN(IntPtr Bnkio, string name, uint flags);
    ...
    */
    ...
    public delegate int  BINKSNDOPEN(IntPtr BnkSnd, uint freq, int bits, int chans, uint flags, IntPtr bink);
    ...
    */
    ...
    public delegate BINKSNDOPEN BINKSNDSYSOPEN(uint param);
    ...
    */
    ...
    {
      public uint Width;                  // Width of frames
      public uint Height;                 // Height of frames
      public uint TotalTime;              // total time (ms)
      public uint FileFrameRate;          // frame rate
      public uint FileFrameRateDiv;       // frame rate divisor
      public uint FrameRate;              // frame rate
      public uint FrameRateDiv;           // frame rate divisor
      public uint TotalOpenTime;          // Time to open and prepare for decompression
      public uint TotalFrames;            // Total Frames
      public uint TotalPlayedFrames;      // Total Frames played
      public uint SkippedFrames;          // Total number of skipped frames
      public uint SkippedBlits;           // Total number of skipped blits
      public uint SoundSkips;             // Total number of sound skips
      public uint TotalBlitTime;          // Total time spent blitting
      public uint TotalReadTime;          // Total time spent reading
      public uint TotalVideoDecompTime;   // Total time spent decompressing video
      public uint TotalAudioDecompTime;   // Total time spent decompressing audio
      public uint TotalIdleReadTime;      // Total time spent reading while idle
      public uint TotalBackReadTime;      // Total time spent reading in background
      public uint TotalReadSpeed;         // Total io speed (bytes/second)
      public uint SlowestFrameTime;       // Slowest single frame time (ms)
      public uint Slowest2FrameTime;      // Second slowest single frame time (ms)
      public uint SlowestFrameNum;        // Slowest single frame number
      public uint Slowest2FrameNum;       // Second slowest single frame number
      public uint AverageDataRate;        // Average data rate of the movie
      public uint AverageFrameSize;       // Average size of the frame
      public uint HighestMemAmount;       // Highest amount of memory allocated
      public uint TotalIOMemory;          // Total extra memory allocated
      public uint HighestIOUsed;          // Highest extra memory actually used
      public uint Highest1SecRate;        // Highest 1 second rate
      public uint Highest1SecFrame;       // Highest 1 second start frame
    };
    ...
    */
    ...
    {
      public uint FrameNum;               // Current frame number
      public uint FrameRate;              // frame rate
      public uint rameRateDiv;           // frame rate divisor
      public uint Frames;                 // frames in this sample period
      public uint FramesTime;             // time is ms for these frames
      public uint FramesVideoDecompTime;  // time decompressing these frames
      public uint FramesAudioDecompTime;  // time decompressing these frames
      public uint FramesReadTime;         // time reading these frames
      public uint FramesIdleReadTime;     // time reading these frames at idle
      public uint FramesThreadReadTime;   // time reading these frames in background
      public uint FramesBlitTime;         // time blitting these frames
      public uint ReadBufferSize;         // size of read buffer
      public uint ReadBufferUsed;         // amount of read buffer currently used
      public uint FramesDataRate;         // data rate for these frames
    };
    ...
    */    
    ...
    public struct BINK_OPEN_OPTIONS
    {
      ...
      public int ForceRateDiv;
      public int IOBufferSize;
      public int Simulate;
      public uint TotTracks;
      [MarshalAs(UnmanagedType.ByValArray, SizeConst = 10)] // Note: kinda nasty, forces you to hand a big int[] to this even if TotTracks is 0. 
      int[] TrackNums;
      BINKIOOPEN UserOpen;
    };
    ...
    */
    public static uint BINKYAINVERT          = 0x00000800; // Reverse Y and A planes when blitting (for debugging)
    public static uint BINKFRAMERATE         = 0x00001000; // Override fr (call BinkFrameRate first)
    public static uint BINKPRELOADALL        = 0x00002000; // Preload the entire animation
    public static uint BINKSNDTRACK          = 0x00004000; // Set the track number to play
    public static uint BINKOLDFRAMEFORMAT    = 0x00008000; // using the old Bink frame format (internal use only)
    public static uint BINKRBINVERT          = 0x00010000; // use reversed R and B planes (internal use only)
    public static uint BINKGRAYSCALE         = 0x00020000; // Force Bink to use grayscale
    //public static uint BINKNOMMX             = 0x00040000; // Don't use MMX
    public static uint BINKNOSKIP            = 0x00080000; // Don't skip frames if falling behind
    public static uint BINKALPHA             = 0x00100000; // Decompress alpha plane (if present)
    public static uint BINKNOFILLIOBUF       = 0x00200000; // Don't Fill the IO buffer (in BinkOpen and BinkCopyTo)
    public static uint BINKSIMULATE          = 0x00400000; // Simulate the speed (call BinkSim first)
    public static uint BINKFILEHANDLE        = 0x00800000; // Use when passing in a file handle
    public static uint BINKIOSIZE            = 0x01000000; // Set an io size (call BinkIOSize first)
    public static uint BINKIOPROCESSOR       = 0x02000000; // Set an io processor (call BinkIO first)
    public static uint BINKFROMMEMORY        = 0x04000000; // Use when passing in a pointer to the file
    public static uint BINKNOTHREADEDIO      = 0x08000000; // Don't use a background thread for IO
    public static uint BINKNOFRAMEBUFFERS    = 0x00000400; // Don't allocate internal frame buffers - application must call BinkRegisterFrameBuffers
    public static uint BINKNOYPLANE          = 0x00000200; // Don't decompress the Y plane (internal flag)
    public static uint BINKRUNNINGASYNC      = 0x00000100; // This frame is decompressing asynchronously
    public static uint BINKWILLLOOP          = 0x00000080; // You are planning to loop this Bink.
    public static uint BINKDONTCLOSETHREADS  = 0x00000040; // Don't close threads on BinkClose (call BinkFreeGlobals to close threads)
    public static uint BINKFILEOFFSET        = 0x00000020; // Use file offset specified by BinkSetFileOffset
    
    public static uint BINKSURFACEFAST       = 0x00000000;
    public static uint BINKSURFACESLOW       = 0x08000000;
    public static uint BINKSURFACEDIRECT     = 0x04000000;
    
    public static uint BINKCOPYALL           = 0x80000000; // copy all pixels (not just changed)
    public static uint BINKCOPY2XH           = 0x10000000; // Force doubling height scaling
    public static uint BINKCOPY2XHI          = 0x20000000; // Force interleaving height scaling
    public static uint BINKCOPY2XW           = 0x30000000; // copy the width zoomed by two
    public static uint BINKCOPY2XWH          = 0x40000000; // copy the width and height zoomed by two
    public static uint BINKCOPY2XWHI         = 0x50000000; // copy the width and height zoomed by two
    public static uint BINKCOPY1XI           = 0x60000000; // copy the width and height zoomed by two
    public static uint BINKCOPYNOSCALING     = 0x70000000; // Force scaling off
    
    //public static uint BINKNOFILLIOBUF     = 0x00200000L; // Don't Fill the IO buffer (in BinkOpen and BinkCopyTo)
    //public static uint BINKALPHA           = 0x00100000L; // Decompress alpha plane (if present)
    //public static uint BINKNOSKIP          = 0x00080000L; // don't skip the blit if behind in sound
    //public static uint BINKNOMMX           = 0x00040000L; // No MMX
    //public static uint BINKGRAYSCALE       = 0x00020000L; // force Bink to use grayscale
    //public static uint BINKRBINVERT        = 0x00010000L; // use reversed R and B planes    
    ...
    #define BINKDOFRAMEHALF1       1
    #define BINKDOFRAMEHALF2       2
    ...
    */
    public static uint BINKSURFACEP8         = 0;
    public static uint BINKSURFACE24         = 1;
    public static uint BINKSURFACE24R        = 2;
    public static uint BINKSURFACE32         = 3;
    public static uint BINKSURFACE32R        = 4;
    public static uint BINKSURFACE32A        = 5;
    public static uint BINKSURFACE32RA       = 6;
    public static uint BINKSURFACE4444       = 7;
    public static uint BINKSURFACE5551       = 8;
    public static uint BINKSURFACE555        = 9;
    public static uint BINKSURFACE565       = 10;
    public static uint BINKSURFACE655       = 11;
    public static uint BINKSURFACE664       = 12;
    public static uint BINKSURFACEYUY2      = 13;
    public static uint BINKSURFACEUYVY      = 14;
    public static uint BINKSURFACEYV12      = 15;
    public static uint BINKSURFACEMASK      = 15;

    public static uint BINKGOTOQUICK         = 1;
    public static uint BINKGOTOQUICKSOUND    = 2;

    public static int BINKGETKEYPREVIOUS    = 0;
    public static int BINKGETKEYNEXT        = 1;
    public static int BINKGETKEYCLOSEST     = 2;
    public static int BINKGETKEYNOTEQUAL  = 128;

    public static uint BINKDOFRAMEHALF1      = 1;
    public static uint BINKDOFRAMEHALF2      = 2;
    public static uint BINKDOFRAMESTART      = 256;
    public static uint BINKDOFRAMEEND        = 512;

    public static uint BINKPLATFORMSOUNDTHREAD      = 1;
    public static uint BINKPLATFORMIOTHREAD         = 2;
    public static uint BINKPLATFORMASYNCTHREADS  = 1024;  // +0 to 7 for which async;
    ...
    public static extern void BinkSetError(string str);
    ...
    public static extern String BinkGetError();
    ...
    public static extern IntPtr BinkOpen(string name,long flags);
    ...
    public static extern IntPtr BinkOpenWithOptions(string name,ref BINK_OPEN_OPTIONS boo, uint flags);
    ...
    public static extern int BinkDoFrame(IntPtr bnk);
    ...
    public static extern int BinkDoFramePlane(IntPtr bnk,uint which_planes);
    ...
    public static extern int BinkWait(IntPtr bnk);
    ...
    public static extern int BinkPause(IntPtr bnk,int pause);
    ...
    public static extern int BinkCopyToBuffer(IntPtr bnk, IntPtr dest, int destpitch, uint destheight, uint destx, uint desty, uint flags);
    ...
    public static extern int BinkCopyToBufferRect(IntPtr bnk, IntPtr dest, int destpitch, uint destheight, uint destx, uint desty, uint srcx, uint srcy, uint srcw, uint srch, uint flags);
    ...
    public static extern int BinkGetRects(IntPtr bnk,uint flags);
    ...
    public static extern void BinkGoto(IntPtr bnk,uint frame,int flags);
    ...
    public static extern uint BinkGetKeyFrame(IntPtr bnk,uint frame,int flags);
    ...
    public static extern int BinkSetVideoOnOff(IntPtr bnk,int onoff);
    ...
    public static extern int BinkSetSoundOnOff(IntPtr bnk,int onoff);
    ...
    public static extern void BinkSetVolume(IntPtr bnk, uint trackid, int volume);
    ...
    public static extern void BinkSetPan(IntPtr bnk,uint trackid, int pan);
    ...
    public static extern int BinkShouldSkip(IntPtr bink);

    //RADEXPFUNC void RADEXPLINK BinkGetPalette( void * out_pal );
    //Not yet implemented

    //#define BINKBGIOSUSPEND 1
    public static uint BINKBGIOSUSPEND         = 1;

    //#define BINKBGIORESUME  2
    public static uint BINKBGIORESUME         = 2;

    //#define BINKBGIOWAIT    0x80000000
    public static uint BINKBGIOWAIT         = 0x80000000;
    ...
    public static extern int BinkControlBackgroundIO(IntPtr bink,uint control);
    ...
    public static extern void BinkSetWillLoop(IntPtr bink,int onoff);
    ...
    public static extern void BinkFlipEndian32( byte[] buffer, uint bytes );
    //#endif
    ...
    */
    ...
    public static extern void BinkOpenTrack(IntPtr bnk,uint trackindex);
    ...
    public static extern uint BinkGetTrackData(ref BINKTRACK bnkt, byte[] dest);
    ...
    public static extern uint BinkGetTrackType(IntPtr bnk,uint trackindex);
    ...
    public static extern uint BinkGetTrackMaxSize(IntPtr bnk,uint trackindex);
    ...
    public static extern uint BinkGetTrackID(IntPtr bnk,uint trackindex);
    ...
    public static extern void BinkGetSummary(IntPtr bnk,ref BINKSUMMARY sum);
    ...
    public static extern void BinkGetRealtime(IntPtr bnk,ref BINKREALTIME run,uint frames);
    ...
    public static extern void BinkSetFileOffset(ulong offset);
    ...
    public static extern void BinkSetSoundTrack(uint total_tracks, uint[] tracks);
    ...
    public static extern void BinkSetFrameRate(uint forcerate, uint forceratediv);
    ...
    public static extern void BinkSetSimulate(uint sim);
    ...
    public static extern void BinkSetIOSize(uint iosize);
    ...
    public static extern int BinkSetSoundSystem(BINKSNDSYSOPEN open, uint param);

    //#ifdef __RADX86__
    //#define BINK_CPU_MMX   1
    public static uint BINK_CPU_MMX = 1;

    //#define BINK_CPU_3DNOW 2
    public static uint BINK_CPU_3DNOW = 2;

    //#define BINK_CPU_SSE   4
    public static uint BINK_CPU_SSE = 3;

    //#define BINK_CPU_SSE2  8
    public static uint BINK_CPU_SSE2 = 8;

    //#elif defined( __RADMAC__ )
    //#define BINK_CPU_ALTIVEC 1
    public static uint BINK_CPU_ALTIVEC = 1;
    ...
    public static extern int BinkControlPlatformFeatures( int use, int dont_use );

    //Not yet implemented
    /*
    #ifdef __RADWIIU__
      RADEXPFUNC void RADEXPLINK BinkSetWiiUFileClient(void* ptr_to_fsclient, void* ptr_to_fscmbblock);
    #endif
      ...
      #define BinkSoundUseSLES() BinkSetSoundSystem(BinkOpenRADSS, 0);
    #endif
    */
    ...
    public static extern BINKSNDOPEN BinkOpenDirectSound(uint param);

    //#define BinkSoundUseDirectSound(lpDS) BinkSetSoundSystem(BinkOpenDirectSound,(UINTa)lpDS)
    public static int BinkSoundUseDirectSound(uint lpDS)
    {
      return BinkSetSoundSystem(BinkOpenDirectSound,lpDS);
    }
    ...
    public static extern BINKSNDOPEN BinkOpenWaveOut(uint param);
    
    //#define BinkSoundUseWaveOut() BinkSetSoundSystem(BinkOpenWaveOut,0)
    public static int BinkSoundUseWaveOut(uint lpDS)
    {
      return BinkSetSoundSystem(BinkOpenWaveOut,lpDS);
    }

    //#endif

    /*
    #if defined( __RADXENON__ ) || defined(__RADWIN__) || defined(__RADSEKRIT__)
      ...
      RADEXPFUNC BINKSNDOPEN RADEXPLINK BinkOpenRADSS(UINTa param); // don't call directly
      #define BinkSoundUseOAL() BinkSetSoundSystem(BinkOpenRADSS, 0);
    ...
    */
    ...
    RADEXPFUNC HBINKBUFFER RADEXPLINK BinkBufferOpen( void* wnd, U32 width, U32 height, U32 bufferflags);
    RADEXPFUNC S32 RADEXPLINK BinkGDSurfaceType( void* gd );
    ...
    RADEXPFUNC HBINKBUFFER RADEXPLINK BinkBufferOpen( void* wnd, U32 width, U32 height, U32 bufferflags);
    RADEXPFUNC S32 RADEXPLINK BinkBufferSetHWND( HBINKBUFFER buf, void* newwnd);
    RADEXPFUNC S32 RADEXPLINK BinkDDSurfaceType(void * lpDDS);
    RADEXPFUNC S32 RADEXPLINK BinkIsSoftwareCursor(void * lpDDSP, void* cur);
    RADEXPFUNC S32 RADEXPLINK BinkCheckCursor(void* wnd,S32 x,S32 y,S32 w,S32 h);
