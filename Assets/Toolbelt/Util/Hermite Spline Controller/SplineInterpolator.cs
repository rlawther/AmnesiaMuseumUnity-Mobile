...
namespace HermiteSplineController {
...
public class SplineInterpolator : MonoBehaviour
{
	...
	protected internal class SplineNode
	{
		...
		internal SplineNode(Vector3 p, Quaternion q, float t, Vector2 io, Vector3 s) { Point = p; Rot = q; Time = t; EaseIO = io; scale = s;}
		internal SplineNode(SplineNode o) { Point = o.Point; Rot = o.Rot; Time = o.Time; EaseIO = o.EaseIO; scale = o.scale;}
	}
	...
	public void AddPoint(Vector3 pos, Quaternion quat, float timeInSeconds, Vector2 easeInOut, Vector3 scale)
	{
		if (mState != "Reset")
			throw new System.Exception("Cannot add points after start");

		mNodes.Add(new SplineNode(pos, quat, timeInSeconds, easeInOut, scale));
	}
	...
	/// Get the tangent to the curve at a particular time.
	/// Technically it's the rotation applied to the vector (1,0,0)
	/// that results in a vector with the same direction as the tangent
	protected Quaternion GetSquadAtTime(float timeParam) 
	{
		float lastNodeTime = mNodes[mNodes.Count - 2].Time;
		if (timeParam >= lastNodeTime) {
			timeParam = lastNodeTime;
		}	
	...
	public Vector3 GetHermiteAtTime(float timeParam, bool wrap = false)
	{
		SplineNode lastNode = mNodes[mNodes.Count - 2];
		if (timeParam >= lastNode.Time) {
			if (wrap) {
					return GetHermiteAtTime(timeParam % lastNode.Time);
			} else {
					return lastNode.Point;
			}
		}
